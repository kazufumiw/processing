...
const vec2 ch_size  = vec2(0.5, 2.5);              // character size (X,Y)
...
const vec2 ch_start = vec2 (ch_space.x * -6., 2.); // start position
      ...
      vec3 ch_color = vec3 (2.0, 2.0, 7.5);        // character color (R,G,B)
...
void main( void )
{
	...
	ch_pos = ch_start + vec2(sin(u_time),0.0);  // set start position
        ...
        _ W E _ C A N _ H E L P   nl1
	W E _ W I L L _ H E L P   nl2
